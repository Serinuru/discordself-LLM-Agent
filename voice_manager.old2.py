...
class VoiceManager:
    ...
    async def handle_voice_state_update(
        ...
        """Join when a real user enters an empty-of-humans channel; leave when
        the channel the bot is in no longer has any non-bot members."""
        if member.bot:
            return

        # Someone joined a voice channel -> join if we're not already connected there
        if after.channel is not None:
            ...
            if voice_client is None:
                ...
                # bot is elsewhere in the guild; only move if that channel is now empty of humans
                if self._human_count(voice_client.channel) == 0:
                    await voice_client.move_to(after.channel)

        # Someone left a voice channel -> check if the bot should leave that one
    ...
    async def handle_message(self, message: discord.Message) -> str:
        """Check a text message for a 'play music <url>' style command. A
        single video plays immediately; a playlist URL queues all its
        entries and starts the first one. Auto-advance for subsequent
        tracks is handled by _play_next's 'after' callback."""
    ...
    async def _play_next(self, guild: discord.Guild) -> None:
        ...
        if not queue or voice_client is None or not voice_client.is_connected():
            return
        ...
        def _after(error):
            ...
            # after() runs on a non-async thread - hop back onto the event loop
